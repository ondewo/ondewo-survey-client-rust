<div align="center">
  ...
  <h1>
  ONDEWO SURVEY Client Rust Library
  </h1>
</div>

This library gives a rust application typed, async access to the ONDEWO SURVEY
(Survey) gRPC API.

It is generated code plus a thin hand-written surface around it. The interface itself is defined
by the protocol buffer files of the [ONDEWO SURVEY API](https://github.com/ondewo/ondewo-survey-api),
...
The library is published to [crates.io](https://crates.io/crates/ondewo-survey-client) as
**`ondewo-survey-client`**, with the API documentation on
[docs.rs](https://docs.rs/ondewo-survey-client). Nothing else is needed to consume it - the stubs are
...
```bash
cargo add ondewo-survey-client
```
...
* Its version tracks the **ONDEWO SURVEY API** it was generated from in major and minor - a crate
...
```bash
git clone --recurse-submodules git@github.com:ondewo/ondewo-survey-client-rust.git
cd ondewo-survey-client-rust
...
module path mirrors the proto package: `package ondewo.survey;` is reachable as
`ondewo_survey_client::ondewo::survey`. Run `cargo doc --open` to browse the exact service
and message names of the API version this crate was generated from.

```rust
use ondewo_survey_client::ondewo::survey::*;
use tonic::transport::Channel;
use tonic::Request;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. A channel to the SURVEY server. `Channel` handles TLS, reconnects and
    //    connection pooling; the generated clients accept any tonic service.
    let channel = Channel::from_static("https://grpc-survey.ondewo.com:443")
        .connect()
        .await?;
    ...
    let token = std::env::var("ONDEWO_SURVEY_TOKEN")?;
...
```text
.
├── ondewo-survey-api                <----- submodule: the proto definitions to compile
...
1. `checkout_defined_submodule_versions` - checks out the pins at the top of the `Makefile`
   (`ONDEWO_SURVEY_API_GIT_BRANCH` and `ONDEWO_PROTO_COMPILER_GIT_BRANCH`)
1. `build_compiler` - builds `ondewo-rust-proto-compiler:latest` from the submodule
1. `update_cargo_version` - writes `ONDEWO_SURVEY_VERSION` into `Cargo.toml`
1. `generate_ondewo_protos` - runs the image over `ondewo-survey-api/ondewo` and writes `src/api`,
...
noticed: messages are serialized and re-parsed field by field, a well-known
`google.protobuf.Struct` is checked to survive a hop with its entries intact, enum discriminants
are pinned, and the generated `FhirServer` is served over a loopback socket and driven by the
generated `FhirClient`, so every declared RPC really is encoded, routed by its
`/ondewo.survey.FHIR/<Method>` path, answered and decoded again. No ONDEWO server is involved.

There is deliberately no explicit-presence case: `ondewo-survey-api` declares no proto3
`optional` scalar anywhere, so there is nothing to assert it against.
...
Releases are cut from the `Makefile`. Bump `ONDEWO_SURVEY_VERSION`, add the matching
