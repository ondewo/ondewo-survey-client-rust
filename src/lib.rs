//! ONDEWO SURVEY (Survey) gRPC client.
//!
//! Everything under [`api`] is GENERATED from the protos of the `ondewo-survey-api` submodule by the
...
//! `package ondewo.survey;` is reachable as `ondewo::survey::<Message>` and each gRPC service
//! as `ondewo::survey::<service>_client::<Service>Client`.
//!
